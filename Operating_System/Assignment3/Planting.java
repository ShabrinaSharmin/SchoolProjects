...
class TA extends Thread
{
	...
	public void incrHoleDug() { 
		try {
			sem.acquire();
			//System.out.println("student"+ (holeDugNum - holeFilledNum) + "<" + MAX);
			if((holeDugNum - holeFilledNum) < MAX) {
				System.out.println("Student: Got the shovel");
				holeDugNum++;
				System.out.println("Student: Hole "+ getHoleDug()+" Dug");
			}
			else
				System.out.println("Student: Must wait for TA "+this.getMAX()+" holes ahead");
	...
	public void incrHoleFilled() {
			...
			try {
				...
				if(holePlantedNum > holeFilledNum) {
					holeFilledNum++;
	...
	public void run()
	{
		while(true)
		{
			...
			try {sleep((int) (100*Math.random()));} catch (Exception e) { break;} // Time to fill hole
			incrHoleFilled();  // hole filled - increment the number	
